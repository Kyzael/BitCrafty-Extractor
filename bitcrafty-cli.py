#!/usr/bin/env python3
"""
BitCrafty-Extractor CLI - Two-pane interface with real-time queue display
Left: Command interface | R        # Create header with provider info and session stats
        header_text = Text()
        header_text.append(f"🤖 Provider: {provider} | ", style="bold cyan")
        header_text.append(f"📋 Queue: {len(self.screenshot_queue)} | ", style="bold yellow")
        header_text.append(f"🍎 Items: {len(self.session_items_found)} | ", style="bold green")
        header_text.append(f"🔨 Crafts: {len(self.session_crafts_found)}\n", style="bold magenta")
        header_text.append("=" * 40 + "\n", style="dim") Live queue status
"""
...
class BitCraftyCLI:
    """Three-pane BitCrafty-Extractor with live queue display and hotkey interface."""
    ...
    def update_command_panel(self):
        ...
        commands_text.append("  This CLI is controlled via global hotkeys\n", style="white")
...
async def main():
    """Entry point for the BitCrafty-Extractor application."""
    cli = BitCraftyCLI()
    await cli.run()
